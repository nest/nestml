...
#define STDP_SYNAPSE_NESTML_IMPL_H
...
// fixed-point math functions from spinn_common
#include "log.h"
...
#define TYPECAST_TO_ACCUM(x) *((accum*)(&(x)))
#define TYPECAST_TO_UINT32_T(x) *((uint32_t*)(&(x)))



// all parameters are externally linked (XXX: into DTCM memory?)
...
/**
 * for a definition of the SpyNNaker "API", see:
 * https://github.com/SpiNNakerManchester/sPyNNaker/blob/master/neural_modelling/src/neuron/plasticity/synapse_dynamics.h
**/
...
// n.b.: this should be a multiple of 32 bits. Bit packing can be used, e.g.
//
...
//    log_info("[NESTML synapse] synapse_structure_get_update_state()");


    extern plasticity_weight_region_data_t *plasticity_weight_region_data;
    update_state.weight_region = &plasticity_weight_region_data[synapse_type];
...
typedef struct {
    ...
    //! The header for this synaptic row
    accum pre_trace__for__stdp_synapse_nestml;
} synapse_row_plastic_header_t;
...
typedef struct fixed_stdp_synapse_nestml {
...
static inline fixed_stdp_synapse_nestml synapse_dynamics_stdp_get_fixed(
...
//    log_info("[NESTML synapse] \tdelay_dendritic = %d, delay_axonal = %d, type = %d, index = %d", delay_dendritic, delay_axonal, synapse_row_sparse_type(control_word, synapse_index_bits, synapse_type_mask), synapse_row_sparse_index(control_word, synapse_index_mask));

    return (fixed_stdp_synapse_nestml) {
       .delay_dendritic = delay_dendritic,
       .delay_axonal = delay_axonal,
       .type = synapse_row_sparse_type(control_word, synapse_index_bits, synapse_type_mask),
       .index = synapse_row_sparse_index(control_word, synapse_index_mask),
...
static inline void synapse_dynamics_stdp_update_ring_buffers(
        weight_t *ring_buffers, fixed_stdp_synapse_nestml s, int32_t weight) {
...
/*    log_info("[NESTML synapse]      ##  printing post window  ##");
    post_event_window_t post_window = post_events_get_window_delayed(post_event_history, begin_time, end_time);

    while (post_window.num_events > 0) {
        const uint32_t delayed_post_time =
                *post_window.next_time + delay_dendritic;
        log_info("[NESTML synapse] \t\t-> post spike: %u, time: %u",
...
#endif // STDP_SYNAPSE_NESTML_IMPL_H