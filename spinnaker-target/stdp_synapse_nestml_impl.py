...
import math
from math import exp # XXX support all NESTML functions that can be used in the initiialisation of parameters and internals
...
def s16_15_to_float(fixp_x: int) -> float:
    """Converts a Q16.15 fixed-point 32-bit integer back to a float.

    The format assumes:
    Bit 31: Sign bit
    Bits 15-30: Integer part
    Bits 0-14: Fractional part (denominator of 2^15)
    """
    # Check if the input is iterable (but ignore strings/bytes)
    if isinstance(fixp_x, Iterable):
        return [s16_15_to_float(x) for x in fixp_x]

    # 1. Extract the sign: bit 31
    # We shift right by 31 to get the lone bit
    sign = (fixp_x >> 31) & 0x1

    # 2. Extract the integer part: bits 15 through 30
    # Masking with 0xFFFF (16 ones) after shifting right by 15
    integer_part = (fixp_x >> 15) & 0xFFFF

    # 3. Extract the fractional part: bits 0 through 14
    # Masking with 0x7FFF (15 ones)
    fractional_part = fixp_x & 0x7FFF

    # 4. Reconstruct the absolute magnitude
    # Fraction is the integer value divided by the scaling factor 2^15
    float_val = integer_part + (fractional_part / (2**15))

    # 5. Apply the sign
    return -float_val if sign == 1 else float_val
...
class stdp_synapse_nestmlDynamics(AbstractPlasticSynapseDynamics):
    ...
    def merge(self, synapse_dynamics: AbstractSynapseDynamics) -> AbstractSynapseDynamics:
        print("[NESTML synapse] trying to merge with: " + str(type(synapse_dynamics)))
    ...
    def get_plastic_synaptic_data(self,
                                  connections: ConnectionsArray,
                                  connection_row_indices: NDArray[integer],
                                  n_rows: int,
                                  n_synapse_types: int,
                                  max_n_synapses: int,
                                  max_atoms_per_core: int) -> Tuple[List[NDArray[uint32]],
                                                                    List[NDArray[uint32]],
                                                                    NDArray[uint32],
                                                                    NDArray[uint32]]:
        """
    ...
    def read_plastic_synaptic_data(
        ...
        connections["weight"] = s16_15_to_float(numpy.concatenate([pp[:size * 4 * n_state_variables].view(uint32)[::n_state_variables] for pp, size in zip(pp_without_headers, fp_size)]).astype(uint32))
