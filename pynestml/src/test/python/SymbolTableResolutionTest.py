...
class SymbolTableResolutionTest(unittest.TestCase):
    ...
    def test(self):
        ...
        res1 = scope.resolveSymbol('test1', SymbolType.VARIABLE)
        assert (res1 is not None)
        res2 = scope.resolveSymbol('testNot', SymbolType.VARIABLE)
        assert (res2 is None)
        res3 = scope.resolveSymbol('test2', SymbolType.VARIABLE)
        assert (res3 is not None and res3.getScopeType() == ScopeType.FUNCTION)
        res4 = scope.resolveSymbol('arg1', SymbolType.VARIABLE)
        assert (res4 is not None and res4.getScopeType() == ScopeType.FUNCTION)
        res5 = scope.resolveSymbol('test3', SymbolType.VARIABLE)
        assert (res5 is not None and res5.getScopeType() == ScopeType.FUNCTION)
        res6 = scope.resolveSymbol('test1', SymbolType.FUNCTION)
        assert (res6 is not None and res6.getScopeType() == ScopeType.GLOBAL)
        res7 = scope.resolveSymbol('test6', SymbolType.VARIABLE)
