...
class ConvolutionsToBuffersTransformer(Transformer):
    ...
    def transform(self,
        ...
        for model in models:
            ...
            metadata[model.name]["delta_factors"] = ASTUtils.get_delta_factors_from_input_port_references(model)
            metadata[model.name]["delta_factors"] |= ASTUtils.get_delta_factors_from_convolutions(model)
