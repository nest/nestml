...
from pynestml.transformers.replace_get_weight_with_buffer_transformer import ReplaceGetWeightWithBufferTransformer
...
def transformers_from_target_name(target_name: str, options: Optional[Mapping[str, Any]] = None) -> Tuple[Sequence[Transformer], Dict[str, Any]]:
    ...
    if target_name.upper() in ["NEST", "SPINNAKER", "PYTHON_STANDALONE", "NEST_COMPARTMENTAL", "NEST_DESKTOP", "GENN", "NEST_GPU"]:
        ...
        replace_get_weight_with_buffer_transformer = ReplaceGetWeightWithBufferTransformer()
        transformers.append(replace_get_weight_with_buffer_transformer)
