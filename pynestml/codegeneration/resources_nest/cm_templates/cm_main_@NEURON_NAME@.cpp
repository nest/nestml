 ...
 *
 */
#include "cm_main_cm_default_nestml.h"
...
template <>
void
DynamicRecordablesMap< cm_main_cm_default_nestml >::create( cm_main_cm_default_nestml& host )
{
...
nest::cm_main_cm_default_nestml::cm_main_cm_default_nestml()
  : ArchivingNode()
  , c_tree_()
  , syn_buffers_( 0 )
  , logger_( *this )
...
nest::cm_main_cm_default_nestml::cm_main_cm_default_nestml( const cm_main_cm_default_nestml& n )
  : ArchivingNode( n )
  , c_tree_( n.c_tree_ )
 ...
 */
void
cm_main_cm_default_nestml::get_status( DictionaryDatum& statusdict ) const
{
  ...
  for ( long comp_idx_ = 0; comp_idx_ != c_tree_.get_size(); comp_idx_++ )
  {
    ...
    CompartmentCmDefaultNestml* compartment = c_tree_.get_compartment( comp_idx_ );
...
void
nest::cm_main_cm_default_nestml::set_status( const DictionaryDatum& statusdict )
{
...
void
nest::cm_main_cm_default_nestml::add_compartment_( DictionaryDatum& dd )
{
...
void
nest::cm_main_cm_default_nestml::add_receptor_( DictionaryDatum& dd )
{
  ...
  CompartmentCmDefaultNestml* compartment = c_tree_.get_compartment( compartment_idx );
  if ( dd->known( names::params ) )
  {
...
void
nest::cm_main_cm_default_nestml::init_recordables_pointers_()
{
  /**
  ...
  for ( auto rec_it = recordables.begin(); rec_it != recordables.end(); rec_it++ )
  {
    ...
    if ( recname_it == recordables_names.end() )
    {
      ...
      recordablesMap_.insert( rec_it->first, DataAccessFunctor< cm_main_cm_default_nestml >( *this, rec_idx ) );
    }
    else
    {
...
void
nest::cm_main_cm_default_nestml::calibrate()
{
  logger_.init();
 ...
 */
void
nest::cm_main_cm_default_nestml::update( Time const& origin, const long from, const long to )
{
...
void
nest::cm_main_cm_default_nestml::handle( SpikeEvent& e )
{
  if ( e.get_weight() < 0 )
  {
...
void
nest::cm_main_cm_default_nestml::handle( CurrentEvent& e )
{
  assert( e.get_delay_steps() > 0 );
  ...
  CompartmentCmDefaultNestml* compartment = c_tree_.get_compartment_opt( e.get_rport() );
  compartment->currents.add_value( e.get_rel_delivery_steps( kernel().simulation_manager.get_slice_origin() ), w * c );
}

void
nest::cm_main_cm_default_nestml::handle( DataLoggingRequest& e )
{
  logger_.handle( e );
}
