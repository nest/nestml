...
class UnitlessExpressionPrinter(ExpressionsPrettyPrinter):
    ...
    def __init__(self, reference_converter=None, types_printer=None):
        ...
        """
        super(UnitlessExpressionPrinter, self).__init__(reference_converter=reference_converter, types_printer=types_printer)
    ...
    def print_expression(self, node, prefix=''):
        ...
        if isinstance(node, ASTSimpleExpression):
            if node.is_numeric_literal():
                ...
                node_is_variable_symbol = node.get_scope().resolve_to_symbol(node.variable.get_complete_name(), SymbolKind.VARIABLE) is not None
