...
:doc:`Section 1 <front>` introduces the model processing frontend, a subsystem which is able to read in a (textual) model and instantiate a computer processable representation. `Section 2 <middle>`_ will subsequently demonstrate a set of assisting components which make interaction with the tool, as well as other tasks, easier to achieve. `Section 3 <back>`_ will then show how model-to-text transformations (i.e., code generation) can be achieved. Finally, for those who are interested in extension points of PyNestML, `Section 4 <extensions>`_ will subsume how the framework has to be adapted and extended to support new concepts in NestML.
