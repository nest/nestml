...
Note that when a continuous-time input port is defined in the synapse model which is connected to a postsynaptic neuron (see :ref:`Third-factor plasticity <synapses_in_nestml#third-factor-plasticity>`), a corresponding buffer is allocated in each neuron which retains the recent history of the connected state variables. Two options are available for the buffer: a "continuous-time" based buffer, or a spike-based buffer (see the NEST code generator option ``continuous_state_buffering_method``).
