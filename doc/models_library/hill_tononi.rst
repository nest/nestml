...
.. code-block:: nestml
   ...
   neuron hill_tononi:
       ...
       equations: # ############
           ...
           inline INaP_thresh mV = -55.7mV # Persistent Na current; member only to allow recording
           inline INaP_slope mV = 7.7mV # Persistent Na current; member only to allow recording
           inline m_inf_NaP real = 1.0 / (1.0 + exp(-(V_m - INaP_thresh) / INaP_slope)) # Persistent Na current; member only to allow recording
           recordable inline I_NaP pA = -NaP_g_peak * m_inf_NaP ** 3 * (V_m - NaP_E_rev)
           inline d_half real = 0.25 # Depol act. K current; member only to allow recording
           inline m_inf_KNa real = 1.0 / (1.0 + (d_half / (IKNa_D / nS)) ** 3.5) # Depol act. K current; member only to allow recording
           ...
           recordable inline I_h pA = -h_g_peak * Ih_m * (V_m - h_E_rev) # The spike current is only activate immediately after a spike.
           ...
           ############
           # I_T
           inline m_inf_T real = 1.0 / (1.0 + exp(-(V_m / mV + 59.0) / 6.2)) # I_KNa
           inline h_inf_T real = 1.0 / (1.0 + exp((V_m / mV + 83.0) / 4)) # I_KNa
           inline tau_m_h real = 1.0 / (exp(-14.59 - 0.086 * V_m / mV) + exp(-1.87 + 0.0701 * V_m / mV)) # I_KNa
           inline D_influx_peak real = 0.025 # yes, 1.25 s
       ...
       update:
           ...
           if (r_potassium > 0) and (r_potassium - 1 == 0): # Deactivate potassium current.
               g_spike = false # Deactivate potassium current.
        
           r_potassium -= 1
           if (not g_spike) and V_m >= Theta: # Set V and Theta to the sodium reversal potential.
               V_m = E_Na
               Theta = E_Na
   ...
   function compute_synapse_constant(Tau_1 msTau_2 msg_peak real) real: # Factor used to account for the missing 1/((1/Tau_2)-(1/Tau_1)) term
...
.. footer::
   ...
   Generated at 2023-03-09 09:13:57.008710