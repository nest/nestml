...
I_shape , currents , tau_syn , C_m , V_abs , I_e , t , e , tau_m , spikes , = symbols(' I_shape currents tau_syn C_m V_abs I_e t e tau_m spikes ')
...
rhsTmp = ((-1))/tau_m*V_abs+1/C_m*(I_shape+I_e+currents)
constantInputs = simplify(1/diff(rhsTmp, I_shape) * (rhsTmp - diff(rhsTmp, V_abs)*V_abs) - (
I_shape
))

# print the definition of the shape
I_shape = 1*(e/tau_syn)*t*exp(((-1))/tau_syn*t)
# also aliases must be defined in terms of new shapes

rhs = ((-1))/tau_m*V_abs+1/C_m*(I_shape+I_e+currents)
...
if dev_t_devV_abs == 0:
    ...
    for shape_index in range(0, len(shapes)):
        ...
        I_shape = symbols("I_shape")
        c2 = diff( ((-1))/tau_m*V_abs+1/C_m*(I_shape+I_e+currents) , I_shape)
    ...
    stateVariablesFile = open('iaf_psc_alpha_multisynapse_neuron.state.variables.tmp', 'w')
    initialValueFile = open('iaf_psc_alpha_multisynapse_neuron.pscInitialValues.tmp', 'w')
    stateVectorTmpDeclarationsFile = open('iaf_psc_alpha_multisynapse_neuron.state.vector.tmp.declarations.tmp', 'w')
    stateVectorUpdateSteps = open('iaf_psc_alpha_multisynapse_neuron.state.vector.update.steps.tmp', 'w')
    stateVectorTmpBackAssignmentsFile = open('iaf_psc_alpha_multisynapse_neuron.state.vector.tmp.back.assignments.tmp', 'w')
    ...
    f = open('iaf_psc_alpha_multisynapse_neuron.P30.tmp', 'w')
    f.write("P30 real = " + str(simplify(c2 / c1 * (exp(__h__ * c1) - 1))) + "# P00 expression")

    propagatorMatrixFile = open('iaf_psc_alpha_multisynapse_neuron.propagator.matrix.tmp', 'w')
    ...
    updateStep = open('iaf_psc_alpha_multisynapse_neuron.propagator.step.tmp', 'w')
    ...
    solverType = open('iaf_psc_alpha_multisynapse_neuron.solverType.tmp', 'w')
    ...
    initialValuesFile = open('iaf_psc_alpha_multisynapse_neuron.pscInitialValues.tmp', 'w')
    implicitFormFile = open('iaf_psc_alpha_multisynapse_neuron.equations.tmp', 'w')
    for shape_index in range(0, len(shapes)):
