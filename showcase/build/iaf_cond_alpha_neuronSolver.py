...
E_L , I_e , t , spikeExc , I_syn_inh , E_ex , e , V_m , spikeInh , E_in , tau_syn_in , tau_syn_ex , I_syn_exc , I_stim , g_in , g_ex , C_m , g_L , I_leak , = symbols(' E_L I_e t spikeExc I_syn_inh E_ex e V_m spikeInh E_in tau_syn_in tau_syn_ex I_syn_exc I_stim g_in g_ex C_m g_L I_leak ')
...
I_syn_exc = g_ex*(V_m-E_ex)
I_syn_inh = g_in*(V_m-E_in)
I_leak = g_L*(V_m-E_L)
rhsTmp = (((-I_leak))-I_syn_exc-I_syn_inh+I_stim+I_e)/C_m
constantInputs = simplify(1/diff(rhsTmp, g_in) * (rhsTmp - diff(rhsTmp, V_m)*V_m) - (
g_in
+ g_ex
))
...
rhs = (((-I_leak))-I_syn_exc-I_syn_inh+I_stim+I_e)/C_m
devV_m = diff(rhs, V_m)
dev_t_devV_m = diff(devV_m, t)
...
if dev_t_devV_m == 0:
    ...
    for shape_index in range(0, len(shapes)):
        ...
        g_in = symbols("g_in")
        c2 = diff( (((-I_leak))-I_syn_exc-I_syn_inh+I_stim+I_e)/C_m , g_in)
    ...
    stateVariablesFile = open('iaf_cond_alpha_neuron.state.variables.tmp', 'w')
    initialValueFile = open('iaf_cond_alpha_neuron.pscInitialValues.tmp', 'w')
    stateVectorTmpDeclarationsFile = open('iaf_cond_alpha_neuron.state.vector.tmp.declarations.tmp', 'w')
    stateVectorUpdateSteps = open('iaf_cond_alpha_neuron.state.vector.update.steps.tmp', 'w')
    stateVectorTmpBackAssignmentsFile = open('iaf_cond_alpha_neuron.state.vector.tmp.back.assignments.tmp', 'w')
    ...
    for shapeIndex in range(0, len(shapes)):
        ...
        stateVectors[orders[shapeIndex], shapeIndex] = V_m
    ...
    f = open('iaf_cond_alpha_neuron.P30.tmp', 'w')
    f.write("P30 real = " + str(simplify(c2 / c1 * (exp(__h__ * c1) - 1))) + "# P00 expression")

    propagatorMatrixFile = open('iaf_cond_alpha_neuron.propagator.matrix.tmp', 'w')
    ...
    updateStep = open('iaf_cond_alpha_neuron.propagator.step.tmp', 'w')
    ...
    updateStep.write("V_m = P30 * (" + str(constantInputs) + ") + " + str(simplify(tmp)) + "\n")

    for shapeIndex in range(0, len(shapes)):
       updateStep.write("V_m += " + str((tmpPropagator[shapeIndex][1:(orders[shapeIndex] + 1),
                                       0:(orders[shapeIndex])] * stateVectors.col(shapeIndex)[0:orders[shapeIndex],
                                                                 0])[orders[shapeIndex] - 1]) + "\n")

    solverType = open('iaf_cond_alpha_neuron.solverType.tmp', 'w')
    ...
    initialValuesFile = open('iaf_cond_alpha_neuron.pscInitialValues.tmp', 'w')
    implicitFormFile = open('iaf_cond_alpha_neuron.equations.tmp', 'w')
    for shape_index in range(0, len(shapes)):
