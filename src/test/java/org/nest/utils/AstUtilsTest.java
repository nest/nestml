...
public class AstUtilsTest extends ModelbasedTest {
  ...
  public void testComputationAliases() {
    ...
    Assert.assertTrue(astCompilationUnit.getNeurons().get(0).findEquationsBlock().isPresent());

    final List<VariableSymbol> aliasesIn = AstUtils.getAliasSymbols(astCompilationUnit.getNeurons().get(0).findEquationsBlock().get());
