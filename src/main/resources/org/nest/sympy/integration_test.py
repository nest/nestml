...
class TestShapesConversionAndPropagatorMatrxiComputation(unittest.TestCase):
    def test_shape_conversion(self):
        shape_alpha = misc.ShapeFunction("shape_alpha", "e / tau * t * exp(-t / tau)")
        self.assertEqual(2, shape_alpha.order)

        shape_exp = misc.ShapeFunction("shape_exp", "exp(-t/tau)")
        ...
        shape_4 = misc.ShapeFunction("shape_4", "cos(t)")
        self.assertEqual(2, shape_4.order)

        shape_5 = misc.ShapeFunction("shape_5", "sin(t)")
        self.assertEqual(2, shape_5.order)

    def test_propagator_matrix(self):
        shape_alpha = misc.ShapeFunction("shape_alpha", "e / tau * t * exp(-t / tau)")
        shape_exp = misc.ShapeFunction("shape_exp", "exp(-t/tau)")
        shape_sin = misc.ShapeFunction("shape_sinb", "sin(t)")
        ...
        # checks that the computation doesn't fail!
