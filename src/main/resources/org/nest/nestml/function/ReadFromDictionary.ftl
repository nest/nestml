...
<#elseif variable.isAlias() && aliasInverter.isInvertableExpression(variable.getDeclaringExpression().get())>
  ...
  <#if baseVariable.isState()>
    if ( updateValue< double >( __d, "${statusNames.name(variable)}", ${variableHelper.printOrigin(baseVariable)}${names.name(baseVariable)} ) ) {
      ${names.setter(baseVariable)}( ${statusNames.getter(baseVariable)}() ${inverseOperation} ${variableHelper.printOrigin(offsetVariable)}${offset});
    }
    else {
      ${names.setter(baseVariable)}( ${statusNames.getter(baseVariable)}() ${inverseOperation} delta_${offset});
    }
  </#if>
