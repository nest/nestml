...
class NestNonLinearDendriteTest(unittest.TestCase):
    ...
    def test_non_linear_dendrite(self):
        ...
        mm.set({"record_from": ["I_dend", "V_m", "dend_curr_enabled", "I_dend_ap"]})
        nest.Connect(mm, nrn)
        ...
        if TEST_PLOTS:
            ...
            ax[0].plot(timevec, I_dend, label="I_dend_syn")
            ax[0].legend()
            ax_ = ax[0].twinx()
        ...
        assert np.all(I_dend[tidx_ap_end:] == 0.), "After dendritic spike, dendritic current should be reset to 0 and stay at 0."
