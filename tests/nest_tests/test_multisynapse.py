...
class TestNestMultiSynapse:
    ...
    def test_multisynapse_with_vector_input_ports(self):
        ...
        sg = nest.Create("spike_generator", params={"spike_times": [20., 80.]})
        nest.Connect(sg, neuron, syn_spec={"receptor_type": receptor_types["SPIKES__VEC_IDX__0"], "weight": 1000., "delay": 0.1})

        sg2 = nest.Create("spike_generator", params={"spike_times": [40., 60.]})
        nest.Connect(sg2, neuron, syn_spec={"receptor_type": receptor_types["SPIKES__VEC_IDX__1"], "weight": 1000., "delay": 0.1})

        sg3 = nest.Create("spike_generator", params={"spike_times": [30., 70.]})
        nest.Connect(sg3, neuron, syn_spec={"receptor_type": receptor_types["SPIKES__VEC_IDX__2"], "weight": 500., "delay": 0.1})

        mm = nest.Create("multimeter", params={"record_from": [
            "I_kernel1__X__spikes__VEC_IDX__0", "I_kernel2__X__spikes__VEC_IDX__1", "I_kernel3__X__spikes__VEC_IDX__2"], "interval": nest.resolution})
        nest.Connect(mm, neuron)
        ...
        if TEST_PLOTS:
            ...
            ax[1].plot(mm.events["times"], mm.events["I_kernel1__X__spikes__VEC_IDX__0"], label="I_kernel0")
            ax[1].set_ylabel("current")

            ax[2].plot(mm.events["times"], mm.events["I_kernel2__X__spikes__VEC_IDX__1"], label="I_kernel1")
            ax[2].set_ylabel("current")

            ax[3].plot(mm.events["times"], mm.events["I_kernel3__X__spikes__VEC_IDX__2"], label="I_kernel2")
            ax[3].set_ylabel("current")
