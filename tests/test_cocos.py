...
class TestCoCos:
    ...
    def test_spiking_input_port_on_equation_rhs_outside_convolve(self):
        model = self._parse_and_validate_model(os.path.join(os.path.realpath(os.path.join(os.path.dirname(__file__), 'resources')), 'spiking_input_port_on_equation_rhs_outside_convolve.nestml'))
        assert len(Logger.get_all_messages_of_level_and_or_node(model, LoggingLevel.ERROR)) > 0

    def test_spiking_input_port_on_equation_rhs_outside_convolve2(self):
        model = self._parse_and_validate_model(os.path.join(os.path.realpath(os.path.join(os.path.dirname(__file__), 'resources')), 'spiking_input_port_on_equation_rhs_outside_convolve2.nestml'))
        assert len(Logger.get_all_messages_of_level_and_or_node(model, LoggingLevel.ERROR)) > 0
    ...
    #     assert any(["Actual type different from expected. Expected: 'pA', got: 'mV'!" in log_entry[2] for log_entry in Logger.get_all_messages_of_level_and_or_node(model, LoggingLevel.ERROR)])
    ...
    # def test_invalid_co_co_spike_input_ports_appear_only_in_equation_rhs_and_event_handlers(self):
