...
class ExpressionTestVisitor(ASTVisitor):
    def endvisit_assignment(self, node):
        ...
        variable_symbolbol = scope.resolve_to_symbol(var_name, SymbolKind.VARIABLE)

        _equals = variable_symbolbol.get_type_symbol().equals(_expr.type) \
            or variable_symbolbol.get_type_symbol().differs_only_in_magnitude(_expr.type)

        message = 'line ' + str(_expr.get_source_position()) + ' : LHS = ' + \
                  variable_symbolbol.get_type_symbol().get_symbol_name() + \
